...
knitr::opts_knit$set(root.dir = "C:/Users/jtsum/OneDrive/2019-0-spring/elasticity_project/rotation3/final_fileset/")
library(tidyverse)
library(plyr)
library(reshape2)
...
ggplot(contrib.cur.sum,aes(x= vital_rate, y=contrib, fill = vital_rate, col = vital_rate))+
  ...
  labs(x="Vital Rate", y = "Contrib. to Var. in Growth Rate")+
...
ggplot(filter(contrib.sum, grepl("P|B|I|F",vital_rate)),
  ...
  labs(x="", y = "Contrib. to Var. in Growth Rate")+
...
env.contrib.plot <- ggplot(filter(contrib.env.sum, coef_bound == "est"), 
  ...
  scale_x_discrete(labels = c("Acorn Abund.", "Burned Area",
                               "Pop. Density", "EQSOI",
                               "Relatedness", "Year")) +
  facet_grid(time~sex, labeller = labeller(sex = c(F = "Female-only Model", 
                                                M = "Male-only Model"),
                                           time = c(acurrent = "Current\nVariance",
                                                    past = "Time-Lagged\nVariance",
                                                cross = "Inter-Annual\nCovariance")))
...
pdf(file = "figures/env_contrib_Demo_20231106.pdf",
   width = 14,
   height = 8.5)
...
env.vr.contrib.plot <- ggplot(filter(contrib.env.melt, coef_bound == "est"), 
       aes(x = factor(substr(vr, 1, 1),
                         levels = c("F", "P", "I", "B")), y = contrib,
           fill = substr(env, 1, 1)))+
  geom_bar(stat = "identity", position = position_stack(reverse = FALSE)) +
  theme_light() +
  theme(axis.text.x = element_text(angle = 35, hjust = 1),
...
env.vr.contrib.plot.c <- ggplot(filter(contrib.env.melt, coef_bound == "est",
                                       time == "acurrent"), 
       aes(x = factor(substr(vr, 1, 1),
                         levels = c("F", "P", "I", "B")), y = contrib,
           fill = substr(env, 1, 1)))+
...
pdf(file = "figures/env_vr_contrib_Demo_20231106.pdf",
   width = 14,
   height = 8.5)
...
vr.contrib.plot <- ggplot(filter(contrib.means, grepl("I|B|F|P", substr(vr, 1, 1)), 
              grepl("I|B|F|P", substr(co_vr, 1, 1))), 
       aes(x = factor(substr(vr, 1, 1), levels = c("F", "P", "I", "B")) , 
           y = contrib, 
           fill = factor(substr(co_vr, 1, 1), levels = c("F", "P", "I", "B")))) +
  geom_bar(stat = "identity") +
  theme_light() + 
  ...
  geom_hline(yintercept = 0,size = 1) +
  labs(y = "Contrib. to Var. in Growth", x = "",
       fill = "Co-Factor Vital Rate") +
  scale_x_discrete(labels = c("Fecundity", "Survival", "Immigration", 
                              "Prob. to Pair")) +
  facet_grid(source~sex, labeller = labeller(sex = c(F = "Female-only Model", 
                                                M = "Male-only Model"),
                                             source = c(acurrent = "Current\nVariance",
                                                    past = "Time-Lagged\nVariance",
                                                cross = "Inter-Annual\nCovariance")))

vr.contrib.plot.c <- ggplot(filter(contrib.means, grepl("I|B|F|P", substr(vr, 1, 1)), 
              grepl("I|B|F|P", substr(co_vr, 1, 1)),
              source == "acurrent"), 
       aes(x = factor(substr(vr, 1, 1), levels = c("F", "P", "I", "B")) , 
           y = contrib, 
...
pdf(file = "figures/vr_contrib_Demo_20231106.pdf",
   width = 14,
   height = 8.5)
...
Create a plot showing the big picture of where contribution to variation in the population growth rate come from.

```{r}
overview.contrib <- ggplot(filter(contrib.sums, grepl("I|B|F|P", vr)), 
       aes(x = factor(vr, levels = c("F", "P", "I", "B")) , 
           y = contrib, 
           fill = vr, alpha = source)) +
  geom_bar(stat = "identity") +
  scale_alpha_discrete(range = c(1,0.3),  
                       labels = c("Environment", "Vital Rates", 
                                  "Demo. Structure")) +
  theme_light() + 
  theme(text = element_text(size = 25),
        axis.text.x = element_text(angle = 35, hjust = 1)) +
  scale_fill_manual(values = rate_colors2, 
                    labels = c("Transition", "Fecundity", 
                               "Immigration", "Survival")) +
  labs(y = "Contrib. to Var. in Growth", x = "",
       fill = "Vital Rate", alpha = "Variance Source") +
  geom_hline(yintercept = 0,size = 1) +
  guides(fill = FALSE) +
  scale_x_discrete(labels = c("Fecundity", "Survival", "Immigration", 
                              "Prob. to Pair")) +
...
pdf(file = "figures/overview_contrib_Demo_20231106.pdf",
   width = 14,
   height = 8.5)
...
## create inbreeding plot

# adjust inbreeding values
AnnualRelatedness <- mutate(AnnualRelatedness, 
                            breeder.unscale = (breeder*pair.Fped.values$sd) + 
                              pair.Fped.values$mean)

inbreeding.plot <- ggplot(AnnualRelatedness, aes(x = Year, y = breeder.unscale)) +
  geom_line(size = 1, col = "#78d151") +
  geom_point(size = 2, col = "#78d151") +
  geom_hline(yintercept = 0, size = 1) + 
  labs(y = "Average Inbreeding Coef. of Breeders") +
  theme_minimal() +
  theme(text = element_text(size = 20))

pdf(file = "figures/inbreeding_plot_20231106.pdf",
   width = 7,
   height = 6)
...
imm.num <- filter(FullLOI, category == "immigrant",
                  Year >= 1988, Year <= 2021) %>%
  group_by(Year) %>%
  dplyr::summarize(immNum = n())



imm.plot <- ggplot(imm.num, aes(x = Year, y = immNum)) +
  geom_line(size = 1, col = "#7a5195") +
  geom_point(size = 2, col = "#7a5195") +
  geom_hline(yintercept = 0, size = 1) +
  labs(y = "# of New Immigrant") +
  theme_minimal() +
  theme(text = element_text(size = 20))


pdf(file = "figures/imm_plot_20231106.pdf",
   width = 7,
   height = 6)
...
Annualvr <- vr.mat.F %>% reshape2::melt() %>%
  mutate(Sex = "F") %>% dplyr::select(Sex, iter = Var1, 
                                       vr = Var2, Year = Var3, value) %>%
  bind_rows(vr.mat.M %>% reshape2::melt() %>% mutate(Sex = "M") %>%
              dplyr::select(Sex, iter = Var1, vr = Var2, Year = Var3, value)) %>%
  group_by(Sex, vr, Year) %>%
  dplyr::summarize(mean = mean(value), sd = sd(value))

ggplot(filter(Annualvr, !vr %in% c("I", "Bb", "Dn", "Do", "Bi")),
       aes(x = Year, y = mean, col = vr)) +
  geom_point() +
  geom_line() +
  geom_ribbon(aes(ymin = mean - sd, ymax = mean + sd,
                  fill = vr), alpha = 0.5) +
  facet_grid(substr(vr, 1, 1)~Sex, scales = "free_y") +
  theme_minimal() +
  theme(text = element_text(size = 20))

# survival plot
pdf(file = "figures/surv_plot_20231106.pdf",
   width = 7,
   height = 6)

ggplot(filter(Annualvr, vr %in% c("Pj", "Ph", "Pb"), Sex == "F"),
       aes(x = Year, y = mean*100, col = vr)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  geom_hline(yintercept = 0, size = 1) +
  labs(x = "Year", y = "% Annual Survival", col = "Social Class") +
  scale_color_manual(values = c("#FF7F00", "#33A02C", "#1F78B4"), 
                     labels = c("Juvenile", "Helper", "Breeder")) +
  theme_minimal() +
  theme(text = element_text(size = 20))
...
# fecundity plot
pdf(file = "figures/fec_plot_20231106.pdf",
   width = 7,
   height = 6)

ggplot(filter(Annualvr, vr %in% c("Fn", "Fo"), Sex == "F"),
       aes(x = Year, y = mean, col = vr)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  geom_hline(yintercept = 0, size = 1) +
  labs(x = "Year", y = "Mean # of Offspring", col = "Pair Status") +
...
pdf(file = "figures/transition_plot_20231106.pdf",
   width = 7,
   height = 6)

ggplot(filter(Annualvr, vr %in% c("Bj", "Bh"), Sex == "M"),
       aes(x = Year, y = mean*100, col = vr)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  geom_hline(yintercept = 0, size = 1) +
  labs(x = "Year", y = "% Become Breeder", col = "Social Class") +
  scale_color_manual(values = c("#FF7F00", "#33A02C"), 
                     labels = c("Juvenile", "Helper")) +
  theme_minimal() +
  theme(text = element_text(size = 20))
...
pdf(file = "figures/GLMM_coef_20231106.pdf",
   width = 10,
   height = 12)

ggplot(filter(model_est, 
              !factor %in% c("(Intercept)",
                             "social.classjuvenile",
                             "SexM", "pair.statusold", "EQSOIdry"),
              !model %in% c("B", "Bi")), 
  ...
  geom_text(aes(label = ifelse(p_value < 0.05 & p_value >=0.01, "*",
                ...
                y = est + se + 0.01),
            position = position_dodge(width = 0.7), size = 7, hjust = 0,
            show.legend=FALSE) +
  coord_flip() +
  scale_x_discrete(labels = c("Failure to Breed", "Offspring #", "Female Immigration",
                              "Male Immigration",
                              "Breeder Surv.", "Helper Surv.", "Juvenile Surv.")) + 
  guides(col = FALSE) + 
  facet_wrap(.~fac, labeller = labeller(fac = c(acorns = "Acorn Abund.",
                                                burn = "Burned Area",
                                                density = "Pop. Density",
                                                EQSOI = "EQSOI",
                                                pedF = "Relatedness",
...
Make a plot of the population growth rate based on the female model


```{r}
# get mean population growth rates
lambda.means <- apply(pop.vec.F, c(1, 3), function(x)
  sum(x)) %>% apply(1, function(x) x[2:length(x)]/x[1:(length(x) - 1)]) %>%
  apply(1, mean)

lambda.df <- data.frame(Year = 1988:2020, lambda = lambda.means)
lambda.geo.mean <- exp(mean(log(lambda.means)))

lambda.plot <- ggplot(lambda.df, aes(x = Year, y = log(lambda))) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_hline(yintercept = 0, size = 1) +
  labs(y = "Population Growth Rate") + 
  geom_hline(yintercept = log(lambda.geo.mean), size = 1, col = "chartreuse3") +
  theme_minimal() +
  theme(text = element_text(size = 20))

pdf(file = "figures/pop_growth_20231106.pdf",
   width = 8.5,
   height = 6)
...
save(contrib.sums, contrib.env.melt, contrib.means, contrib.t.F, contrib.t.M,
     ...
     file = "data/generated_data/Demo_LTRE_results.rdata")
